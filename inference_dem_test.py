...
ONNX_MODEL = os.path.join('onnx_models', 'UN_norm.onnx')
INPUT_FOLDER = os.path.join('input', 'deminput')
...
def preprocess(image):
    ...
    R = np.nan_to_num(np.array(image[:, :, 0], dtype=np.float32),
                      nan=0)
...
def main():
    ...
    output_filename = f'elimina.png'
    image_s = tifffile.imread(os.path.join(INPUT_FOLDER, '135.tif'))
    image_d = tifffile.imread(os.path.join(INPUT_FOLDER, '135l.tif'))
    ...
    print(0, prediction_image.shape)
    prediction_image_final = np.argmax(prediction_image, axis=2)
